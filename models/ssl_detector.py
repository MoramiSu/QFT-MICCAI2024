        ...
        if split != "train":
            ...
            if split == "val":
                ...
                for i in range(len(targets)):
                    ...
                    filtered_target = target[target[:, 3] > 0]
                    if filtered_target.shape[0] > 0:
                        for gt_box in filtered_target:
                            gt_bbox = gt_box.cpu().numpy()
                            x_min, y_min, x_max, y_max = gt_bbox[1:]
                            cv2.rectangle(image, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 255, 0), 2)

                    if out is not None:
                        for pred_box in out:
                            pred_box = pred_box.cpu().numpy()
                            x_min, y_min, x_max, y_max = pred_box[:4]
                            cv2.rectangle(image, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 0, 255), 2)

                    # Add label text above the bounding box
                    # label_text = f"{label}"
                    # label_size, _ = cv2.getTextSize(label_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
                    # cv2.putText(image, label_text, (x_min, y_min - label_size[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    #             (0, 255, 0), 1)

                    gt_output_path = str(BUSI_DETECTION_VISUAL_RES_GLORIA / batch['filenames'][i])
